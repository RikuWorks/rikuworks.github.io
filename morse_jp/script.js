...
function addSymbol(sym) {
  ...
  const tentative = currentSignal + sym;

  if (!correctSignal.startsWith(tentative)) {
    missCount++;

    const info = IROHA_INFO[currentChar] || {};
    feedbackElem.textContent =
      `❌ 不正解\n正解: ${correctSignal}\n覚え方: ${info.mnemonic} 漢字: ${info.kanji}`;

    waitingNext = true;
    updateScore();
    setTimeout(() => { waitingNext = false; nextQuestion(); }, 2000);
    return;
  }

  currentSignal = tentative;
...
function updateScore() {
  scoreElem.textContent =
    `正解: ${correctCount} ミス: ${missCount} 進捗: ${currentIndex}/${IROHA_SEQ.length}`;
}
...
dotBtn.addEventListener("click", () => { addSymbol("・"); playClickTone(); });
dashBtn.addEventListener("click", () => { addSymbol("－"); playClickTone(); });

let pressStartTime = null;
let spaceKeyDown = false;

window.addEventListener("keydown", (e) => {
  if (e.code === "Space" && !spaceKeyDown && gameRunning && !waitingNext) {
    e.preventDefault();
    spaceKeyDown = true;
...
window.addEventListener("keyup", (e) => {
  if (e.code === "Space" && spaceKeyDown && gameRunning && !waitingNext) {
    ...
    const dur = (performance.now() - pressStartTime) / 1000;
    stopTone();
    addSymbol(dur < DOT_THRESHOLD ? "・" : "－");
  }
});

// ツイート
function tweetResult() {
  const total = IROHA_SEQ.length;
  ...
  const text =
    ...
    `ミス: ${missCount}\n正答率: ${ratio}%\n` +
    `#和文モールス #モールス信号 #和文モールスいろはにほへと\n`+
    `https://rikuworks.github.io/morse_jp/morse_jp.html`;

  const url = `https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}`;
  window.open(url, "_blank");
}
tweetBtn.addEventListener("click", tweetResult);
